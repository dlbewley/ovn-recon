...
> [!IMPORTANT]
> **Partially superseded.** The multi-arch *requirement* and the failure modes described here are
> still accurate and still enforced. The *mechanism* changed: the catalog is now a
> [File-Based Catalog](fbc-migration.md) built from `operator/catalog.Dockerfile`, not a sqlite
> index generated by `opm index add --generate`.
>
> What that changes in this document:
>
> | This document says | Current reality |
> |---|---|
> | `make catalog-build-multiarch` | `make catalog-fbc-build-multiarch` |
> | `make catalog-index-dockerfile` | removed — no generated Dockerfile; `catalog.Dockerfile` is committed |
> | `make catalog-push-pruned-index`, `opm index rm` | removed — pruning is a text edit to `catalog.json` |
> | `OPM_INDEX_BINARY_IMAGE` pins the generated `FROM` line | `OPM_IMAGE` pins `catalog.Dockerfile`'s base, **already pinned by digest** |
> | `BUNDLE_IMGS`, `CATALOG_BASE_IMG`, `FROM_INDEX` | removed — the catalog is the committed content, not the previously published image pulled forward |
>
> The "Pruning stale `operator` package" section below is retained as a record of how that was done
> under sqlite; the targets it references no longer exist.
>
> Still current and worth reading: the arm64 crash symptoms, the docker-buildx vs podman-manifest
> split (the FBC target branches on `CONTAINER_TOOL` the same way), the verification recipes, and
> the release validation checklist.
...
**Done (2026-08-18):** the catalog base image is now pinned by digest via **`OPM_IMAGE`** in
`operator/Makefile` and `operator/catalog.Dockerfile`. The original recommendation follows for
context.

**Original recommended hardening step:** set **`OPM_INDEX_BINARY_IMAGE`** to a
